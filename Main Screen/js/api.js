...
function getFinantial(){
    ...
    $.ajax({
        ...
        finID = setTimeout(getFinantial, 60000 * 5);
      }
...
function getTime(){
    ...
    // $elclock = $('<iframe></iframe>');
    // $.ajax({
    //     url: `${PROXY_URL}${CLOCK_URL}`,
    //     method: 'GET',
    //     success: function(data){
    //             var page = $elclock.html(data).get(0);
    //             var target = page.querySelector('#relogio');
    //             var timeNow = target.innerText;
    //             var hourArray = timeNow.split(":").slice(0, 2);
    //             var time = hourArray.join(":");
...
function getDay(){
    ...
    // var paraDay = document.createElement('p');
    // var paraMonth = document.createElement('p');
    var paraDay = document.getElementById('paraDay');
    var paraMonth = document.getElementById('paraMonth')
    paraDay.innerHTML = dayName;
    paraMonth.innerHTML = dayMonth;
    // dayEl.appendChild(paraDay);
    // dayEl.appendChild(paraMonth);
}
